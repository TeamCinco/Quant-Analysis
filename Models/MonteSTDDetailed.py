...
def monte_carlo_simulation(current_price, projected_daily_return, stdReturn, T, mc_sims=1000000):
    ...
    portfolio_sims[0] = current_price
    for m in range(0, mc_sims):
        Z = np.random.normal(size=T-1)
        dailyReturns = projected_daily_return + stdReturn * Z
        portfolio_sims[1:, m] = current_price * np.cumprod(1 + dailyReturns)
...
def main():
    ...
    filtered_paths = {}
    for sim, labels in path_analysis[:1000]:
    ...
    plt.ylim(current_price * 0.9, current_price * 1.1)  # Set y-axis limits to +/- 10% of current price
